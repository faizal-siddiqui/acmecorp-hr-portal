...
async def main():
    ...
    async with httpx.AsyncClient(transport=httpx.ASGITransport(app=app), base_url="http://test") as client:
        ...
        search_res = await benchmark_endpoint(client, "Employees Search (q=John)", "/employees/?q=John")

        # 5. Benchmark /analytics/summary with filter
        analytics_filter_res = await benchmark_endpoint(client, "Analytics Summary (country=US)", "/analytics/summary?country=US")
