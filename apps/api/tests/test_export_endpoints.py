...
async def test_export_employees_csv_success(client: AsyncClient, db_session: AsyncSession):
    ...
    response = await client.get(
        "/export/employees.csv", headers={"Authorization": f"Bearer {token}"}
    )
...
async def test_export_employees_csv_with_filters(client: AsyncClient, db_session: AsyncSession):
    ...
    response = await client.get(
        "/export/employees.csv?department=Engineering", headers={"Authorization": f"Bearer {token}"}
    )
    assert response.status_code == 200
    ...
    response = await client.get(
        "/export/employees.csv?country=DE", headers={"Authorization": f"Bearer {token}"}
    )
    assert response.status_code == 200
