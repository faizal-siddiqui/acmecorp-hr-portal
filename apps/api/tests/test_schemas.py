...
def test_compensation_update_invalid_base():
    with pytest.raises(ValidationError) as excinfo:
        CompensationUpdate(
            base_annual=0, bonus_annual=10000, currency="USD", effective_date="2026-01-01"
        )
...
def test_compensation_update_invalid_bonus():
    with pytest.raises(ValidationError) as excinfo:
        CompensationUpdate(
            base_annual=100000, bonus_annual=-1, currency="USD", effective_date="2026-01-01"
        )
...
def test_compensation_update_invalid_currency():
    with pytest.raises(ValidationError) as excinfo:
        CompensationUpdate(
            base_annual=100000, bonus_annual=10000, currency="US", effective_date="2026-01-01"
        )
    assert "String should have at least 3 characters" in str(excinfo.value)

    with pytest.raises(ValidationError) as excinfo:
        CompensationUpdate(
            base_annual=100000, bonus_annual=10000, currency="USDD", effective_date="2026-01-01"
        )
