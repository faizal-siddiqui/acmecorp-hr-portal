...
import { updateCompensation, EmployeeDetail } from "@/lib/api";
...
describe("CompensationEditForm", () => {
  ...
  it("renders with initial values", () => {
    render(<CompensationEditForm employee={mockEmployee} onClose={vi.fn()} onSuccess={vi.fn()} />);
  ...
  it("submits successfully", async () => {
    ...
    vi.mocked(updateCompensation).mockResolvedValue({ ...mockEmployee, base_annual: 120000 });
  ...
  it("handles submission error", async () => {
    vi.mocked(updateCompensation).mockRejectedValue(new Error("API Error"));
